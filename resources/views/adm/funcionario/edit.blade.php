@section('titulo', 'Cadastro de Funcionario')
  ...
  @section('content')
    <form action="{{ route('adm.funcionario.update', $funcionario->id) }}" method="POST">
      ...
      <input type="text" class="form-control mb-2" name="nome_func" value="{{ $funcionario->nome_func }}" placeholder="Nome">
      <div class="input-group mb-2">
          <span class="input-group-text">@</span>
          <input type="email" class="form-control" name="email" value="{{ $funcionario->email }}" placeholder="E-mail">
      </div>
