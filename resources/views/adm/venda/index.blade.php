            ...
            <tbody>
                @forelse ($vendas as $venda)
                    @php
                        ...
                        $total_quantidade = 0;
                    @endphp
                    <div class="modal fade" id="venda-show-{{ $venda->id_venda}}" tabindex="-1" aria-labelledby="venda-modal-label" aria-hidden="true">
                                ...
                                @forelse($venda->carrinho->produtos as $item)
                                    ...
                                    <li class="list-group-item">
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <span>R$ {{ number_format($item->preco_produto * $item->pivot->quantidade_item, 2, ',', '.') }}</span>
                                        </div>
                                    </li>
                                @empty
                    ...
                    <tr>
                        ...
                        <td>{{ $venda->pagamento_venda }}</td>
                        <td>{{ $venda->entrega_venda }}</td>
                        <td>R$ {{$total_preco}}</td>
